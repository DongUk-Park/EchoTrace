...
def sasrec_main(dataset, path=None, item_num=None):
    ...
    # global dataset
    #create_name_dict_from_OMDB(data_dir='/home/parkdw00/Codes/A-LLMRec/data/ml-100k', output_dir='/home/parkdw00/Codes/A-LLMRec/data/ml-100k',)
        ...
        if epoch == args.num_epochs:
            ...
            if not os.path.exists(os.path.join(folder, fname)):
                try:
                    ...
                    print()
            os.makedirs(os.path.join(f"/home/parkdw00/Codes/A-LLMRec/pre_train/sasrec/{args.dataset}"), exist_ok=True)
            torch.save([model.kwargs, model.state_dict()], os.path.join(f"/home/parkdw00/Codes/A-LLMRec/pre_train/sasrec/{args.dataset}", fname))
            
        # ----- 파트별 임베딩 저장 (epoch별 X) : 환경변수로 제어 -----
    part_env = os.getenv("SASREC_PART_ID")  # 예: "0", "1", ...
    save_dir = os.getenv("SASREC_EMB_DIR", f"/home/parkdw00/Codes/data/{args.dataset}/A-LLMRec_format/u_i_embedding_in_sasrec")
...
if __name__ == '__main__':
    dataset = "ml-1m"
    train_txt_path = "/home/parkdw00/Codes/data/ml-1m/A-LLMRec_format/ml-1m.txt"
    item_num = 3693
